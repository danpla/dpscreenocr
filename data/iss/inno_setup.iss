...
Source: "{#TESSERACT_DATA_DIR}\eng.traineddata"; \
  DestDir: "{app}\{#TESSERACT_DATA_DIR}"; \
  Check: ShouldInstallEngTessdata(); \
  Flags: ignoreversion
Source: "{#TESSERACT_DATA_DIR}\eng.traineddata"; \
  DestDir: "{localappdata}\{#APP_FILE_NAME}\{#TESSERACT_DATA_DIR}"; \
  Check: ShouldCopyEngTessdataForCurrentUser(); \
...
// In the previous app versions that use the installer generated by
// "cpack -G NSIS", the users have to put custom Tesseract data in the
// "tessdata" directory located in the installation path.
//
// Since version 1.1.0 (which now uses Inno Setup), the "tessdata"
// directory was renamed to "tesseract_data". Instead of loading data
// directly from "tesseract_data", the application now just copies
// this directory to the users local app data on the first start.
//
// When updating from an old NSIS-based version, or goal is to migrate
...
// from "tessdata", but also those that were in "{app}\tesseract_data"
...
//
// Note that we don't remove the files in the "tessdata" directory
// after copying them. We can't be sure the user no longer needs them.
//
// The [Files] entry is executed after the old version is removed, so
...
// NsisWasEngTesseractData, which tells whether the eng data existed;
...
function ShouldInstallEngTessdata(): Boolean;
begin
  Result := IsAdminInstallMode()
    and ((NsisInstallDirPath = '') or NsisWasEngTesseractData);
end;

function ShouldCopyEngTessdataForCurrentUser(): Boolean;
begin
...
begin
  ...
  begin
    ...
    NsisWasEngTesseractData :=
      (NsisInstallDirPath <> '')
      and FileExists(
        NsisInstallDirPath + '\tessdata\eng.traineddata');
